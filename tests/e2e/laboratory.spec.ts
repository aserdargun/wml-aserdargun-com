...
test('guided story reaches measured error and update', async ({ page }) => {
  ...
  await expect(page.locator('.guide-finish')).toHaveText('Gözle. Hayal et. Tahmin et. Uygula. Öğren.')
  await expect(page.locator('.guided-copy')).toContainText('model parametrelerini değiştirmez')
